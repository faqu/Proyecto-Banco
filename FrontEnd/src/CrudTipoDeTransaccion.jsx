...
const CrudTipoDeTransaccion = () => {
    ...
    const handleCreateOrUpdate = async () => {
        try {
            ...
            if (selectedRoleId) {
                ...
                await fetch('http://localhost:32768/api/TipoDeTransaccions/${selectedRoleId}', {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({ tipoTrasaccion: newRoleName }),
                });
            } else {
                // Otherwise, perform create (Crear)
                await fetch('http://localhost:32768/api/TipoDeTransaccions/', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    const handleLogin = () => {
        // Add logic for logout, and then redirect to the login page
        // For now, let's assume the logic for logout is just navigating to the login page
        navigate('/CrudLogin'); // Replace '/login' with the actual path of your login page
    };
    const handleRoles = () => {
        // Add logic for logout, and then redirect to the login page
        // For now, let's assume the logic for logout is just navigating to the login page
                ...
                <List>
                    {data.map((item) => (
                        <ListItem key={item.id}>
                            {item.tipoTrasaccion}
                            <ActionButtons>
                                <ActionButton onClick={() => handlePut(item.id)}>
                                    Actulizar
                                </ActionButton>
                            </ActionButtons>
                        </ListItem>
...
export default CrudTipoDeTransaccion;
